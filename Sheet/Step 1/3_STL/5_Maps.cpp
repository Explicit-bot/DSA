/*
====================================================================
1️⃣ WHAT IS std::map?
====================================================================
std::map is an ordered associative container that stores elements
in the form of (KEY, VALUE) pairs.

• Keys are UNIQUE
• Elements are STORED IN SORTED ORDER (by key)
• Implemented using RED-BLACK TREE (self-balancing BST)
• No random access (not index-based)

------------------------------------------------------------
Header:
------------------------------------------------------------
#include <map>

------------------------------------------------------------
Basic Syntax:
------------------------------------------------------------
map<KeyType, ValueType> mp;

Example:
------------------------------------------------------------
map<int, int> mp;
map<string, int> freq;

====================================================================
2️⃣ INTERNAL IMPLEMENTATION
====================================================================
• Data Structure: Red-Black Tree
• Height of tree: O(log n)
• All major operations depend on tree height

Why important?
------------------------------------------------------------
Because EVERY operation involves:
• Tree traversal
• Key comparison

====================================================================
3️⃣ KEY RULE (VERY IMPORTANT)
====================================================================
KEY:
------------------------------------------------------------
• Must be UNIQUE
• Must be COMPARABLE using operator <

VALUE:
------------------------------------------------------------
• Can be ANY datatype
• No restrictions

Golden Rule:
------------------------------------------------------------
KEY → must support <
VALUE → anything

====================================================================
4️⃣ POSSIBLE DATA TYPES
====================================================================

------------------------------------------------------------
Common KEY types:
------------------------------------------------------------
int
long long
char
string
pair<int,int>
vector<int>        // allowed but slow
custom struct      // only if < is defined

------------------------------------------------------------
Examples:
------------------------------------------------------------
map<int,int>
map<long long,string>
map<string,int>
map<pair<int,int>,int>
map<int,vector<int>>
map<string,map<int,int>>

------------------------------------------------------------
Custom struct as key:
------------------------------------------------------------
struct Node{
    int x, y;
    bool operator<(const Node& other) const {
        if(x != other.x) return x < other.x;
        return y < other.y;
    }
};

map<Node, int> mp;

====================================================================
5️⃣ INSERTION
====================================================================

------------------------------------------------------------
Method 1: operator[]
------------------------------------------------------------
mp[1] = 10;

• Inserts if key not present
• Updates if key exists

Time: O(log n)

------------------------------------------------------------
Method 2: insert()
------------------------------------------------------------
mp.insert({2, 20});

------------------------------------------------------------
Method 3: emplace()
------------------------------------------------------------
mp.emplace(3, 30);

✔ Slightly faster (no temporary object)

====================================================================
6️⃣ ACCESSING ELEMENTS
====================================================================

------------------------------------------------------------
Using operator[]
------------------------------------------------------------
cout << mp[1];

⚠️ If key doesn't exist:
• New key is created
• Default value is assigned

------------------------------------------------------------
Safe access using find():
------------------------------------------------------------
if(mp.find(1) != mp.end()){
    cout << mp[1];
}

====================================================================
7️⃣ find() FUNCTION
====================================================================

------------------------------------------------------------
Syntax:
------------------------------------------------------------
auto it = mp.find(key);

• Returns iterator if found
• Returns mp.end() if not found

Time:
------------------------------------------------------------
O(log n)

====================================================================
8️⃣ count()
====================================================================

------------------------------------------------------------
Syntax:
------------------------------------------------------------
mp.count(key);

• Returns 1 if key exists
• Returns 0 otherwise

Time:
------------------------------------------------------------
O(log n)

====================================================================
9️⃣ ERASE (VERY IMPORTANT)
====================================================================

------------------------------------------------------------
Erase by key:
------------------------------------------------------------
mp.erase(key);

Time:
------------------------------------------------------------
O(log n)

------------------------------------------------------------
Erase by iterator:
------------------------------------------------------------
auto it = mp.find(key);
mp.erase(it);

Time:
------------------------------------------------------------
O(1) amortized (after find)

------------------------------------------------------------
Erase range:
------------------------------------------------------------
mp.erase(mp.begin(), mp.end());

Time:
------------------------------------------------------------
O(n)

------------------------------------------------------------
Safe erase while iterating:
------------------------------------------------------------
for(auto it = mp.begin(); it != mp.end(); ){
    if(it->first % 2 == 0)
        it = mp.erase(it);
    else
        ++it;
}

====================================================================
🔟 TRAVERSAL
====================================================================

------------------------------------------------------------
Iterator traversal:
------------------------------------------------------------
for(auto it = mp.begin(); it != mp.end(); ++it){
    cout << it->first << " " << it->second << endl;
}

------------------------------------------------------------
Range-based loop (recommended):
------------------------------------------------------------
for(auto &p : mp){
    cout << p.first << " " << p.second << endl;
}

------------------------------------------------------------
Reverse traversal:
------------------------------------------------------------
for(auto it = mp.rbegin(); it != mp.rend(); ++it){
    cout << it->first << endl;
}

====================================================================
1️⃣1️⃣ ITERATORS & MOVEMENT
====================================================================

• map iterators are BIDIRECTIONAL

------------------------------------------------------------
Valid:
------------------------------------------------------------
++it
--it

------------------------------------------------------------
Invalid:
------------------------------------------------------------
it + 1   // ❌ NOT allowed

Reason:
------------------------------------------------------------
map is NOT random-access

====================================================================
1️⃣2️⃣ SIZE, EMPTY, CLEAR
====================================================================

------------------------------------------------------------
size():
------------------------------------------------------------
mp.size();     // O(1)

------------------------------------------------------------
empty():
------------------------------------------------------------
mp.empty();    // O(1)

------------------------------------------------------------
clear():
------------------------------------------------------------
mp.clear();    // O(n)

====================================================================
1️⃣3️⃣ TIME COMPLEXITY SUMMARY
====================================================================

Operation                Complexity
------------------------------------------------------------
insert                   O(log n)
erase(key)               O(log n)
erase(iterator)          O(1)
find                     O(log n)
count                    O(log n)
mp[key]                  O(log n)
traversal                O(n)
clear                    O(n)

====================================================================
1️⃣4️⃣ STRING AS KEY (SPECIAL CASE)
====================================================================

Let:
------------------------------------------------------------
n = number of elements
L = length of string key

------------------------------------------------------------
Why extra cost?
------------------------------------------------------------
• Tree comparison = O(log n)
• String comparison = O(L)

------------------------------------------------------------
Final complexity:
------------------------------------------------------------
insert / find / erase:
O(log n × L)

====================================================================
1️⃣5️⃣ WHEN TO USE map
====================================================================

✔ Need sorted order
✔ Need lower_bound / upper_bound
✔ Lexicographical traversal
✔ Deterministic performance

====================================================================
1️⃣6️⃣ COMMON MISTAKES
====================================================================

❌ Expecting O(1) access
❌ Using it+1
❌ Using mp[key] just to check existence
❌ Using map<string> for huge data
❌ Modifying map incorrectly while iterating

====================================================================
1️⃣7️⃣ CP TIPS
====================================================================

• Prefer unordered_map for frequency counting
• Use map when ordering matters
• Avoid string keys if possible
• Use pair<int,int> instead of string when you can

====================================================================
1️⃣8️⃣ INTERVIEW ONE-LINER
====================================================================

std::map is an ordered associative container implemented
using a Red-Black Tree, providing O(log n) insert, delete,
and search with keys stored in sorted order.

********************************************************************/
